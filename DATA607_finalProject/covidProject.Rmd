...
&nbsp;&nbsp;&nbsp;&nbsp;The motivation of this analysis is to present numbers and visualizations to clarify the impact that the COVID-19 pandemic has had on the US thus far (the data used in this analysis was updated asrecently as May 8th 2020).
...
#use URL to read data to R data.frames
...
#read to an R data.frame
top15Diseases <- data.frame(read.delim( top15Diseases_URL, sep = "\t", header = T ))
top15Diseases <- top15Diseases[1:15,] #select the 15 rows of text that hold data of interest
names( top15Diseases ) <- c("rowNum", "Causes", "GRCode", "Deaths","Population", "CrudeRate") #name columns
top15Diseases <- top15Diseases %>%
  select( -rowNum, -GRCode ) #exclude unnecessary rows
#display sample of top15Disease data.frame
head(top15Diseases)
```
...
total5820 <- COVID_DeathbyCounty_df %>%
  filter( County.Name != 'Statewide Unallocated') %>% #make sure it has an acceptable fips
  select( X5.8.20 ) #select May 8th totals
p1 <- ggplot( total5820, aes(y=X5.8.20) ) +
  geom_boxplot(fill = fillcolor, alpha = 0.2) + #boxplot of the data
  scale_y_log10() + #log axis
  xlab( "US Counties") +
  ylab( "#Deaths (log10)") +
  labs( title="Boxplot: Deaths by US County")
p2 <- ggplot( total5820, aes(x=X5.8.20)) + #Plot a histogram
...
#find and display the 15 counties with the most deaths in the US
...
```
```{r}
#display summary statistics of county reported COVID19 deaths
...
#usmap to display death totals for counties
...
#visualize the distribution as a box & histogram
...
#display the most populous counties in the US
...
#visualize county population with usmap methods
...
countyMortality <- COVID_DeathbyCounty_df %>%
  mutate( countyPop = COVID_CountyPop_df$countyPop ) %>% #add a column with county population data
  filter( County.Name != 'Statewide Unallocated', County.Name != 'New York City Unallocated/Probable') %>%
  select( countyPop, X5.8.20, countyFIPS, County.Name, State ) %>%
  mutate( mortRate = X5.8.20/countyPop*100000) %>% #calculate a crude mortality rate
  rename( fips = countyFIPS) %>%
  mutate( fips = as.character( fips ) ) %>%
  mutate( fips = ifelse( nchar( fips ) ==4, paste( '0', fips, sep = ""), fips ) ) #fips has to be properly formatted for usmaps methods

#which counties have the highest mortalities?
...
#plot the distribution of mortality rates
...
The code is available here, but does not execute in this document. Rather the images were collated as a .gif outside of the RStudio environment.
```{r, message=F}
...
And now to shift focus to the second data source.
...
```{r, warning=F, message=F}
top15Diseases$Causes <- str_extract( top15Diseases$Causes, '[A-Z].*[a-z]') #clear the Causes feature text

#bar plots for total deaths by leading diseases and mortality rates for leading diseases
...
```{r, message=F}
#plot a messy line scatter plot to visualize the number of counties the cross leading disease thresholds
...
1. [livescience.com](https://www.livescience.com/coronavirus-leading-cause-of-death.html)
2. [Wall Street Journal](https://www.wsj.com/articles/leading-cause-of-death-in-u-s-hint-it-isnt-covid-19-11588930202)
3. [USAFacts.org](https://usafacts.org/issues/coronavirus/)
4. [CDC's WONDER](https://wonder.cdc.gov/)
5. [The Becker's Hospital Review](https://www.beckershospitalreview.com/public-health/covid-19-is-3rd-leading-cause-of-death-stat-analysis-suggests.html)
6. [NYT Coronavirus Deathtoll](https://www.nytimes.com/interactive/2020/05/05/us/coronavirus-death-toll-us.html)
7. [Berkeley News COVID19 fatality rates](https://news.berkeley.edu/2020/04/24/study-challenges-reports-of-low-fatality-rate-for-covid-19/)
